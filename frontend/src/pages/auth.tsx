  ...
  return (
    <>
      <div style={{ minHeight: '100vh', background: 'var(--qa-bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px 80px', position: 'relative', overflow: 'hidden', transition: 'background 0.25s ease' }}>
        {/* Gradient orbs */}
        <div style={{ position: 'absolute', width: '520px', height: '520px', borderRadius: '50%', background: '#3B82F6', filter: 'blur(90px)', opacity: orbOpacity, top: '-140px', left: '-140px', animation: 'float 14s ease-in-out infinite', pointerEvents: 'none', transition: 'opacity 0.25s ease' }} />
        <div style={{ position: 'absolute', width: '420px', height: '420px', borderRadius: '50%', background: '#60A5FA', filter: 'blur(90px)', opacity: isDark ? 0.3 : 0.1, bottom: '-120px', right: '-120px', animation: 'float 14s ease-in-out infinite reverse', pointerEvents: 'none', transition: 'opacity 0.25s ease' }} />
        {/* Dot grid overlay */}
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(59,130,246,0.1) 1px, transparent 0)', backgroundSize: '28px 28px', pointerEvents: 'none' }} />

        <div className="fu" style={{ position: 'relative', zIndex: 1, textAlign: 'center', width: '100%', maxWidth: '820px' }}>
          <div style={{ fontSize: '11px', letterSpacing: '0.3em', color: 'var(--qa-text-mid)', fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginBottom: '18px', textTransform: 'uppercase' }}>O2H TECHNOLOGY</div>
          <h1 style={{ fontSize: 'clamp(44px,7vw,72px)', fontWeight: 800, letterSpacing: '-0.03em', ...titleStyle, margin: '0 0 10px', fontFamily: "var(--font-heading, 'Montserrat', sans-serif)", lineHeight: 1.02 }}>
            Quality <span style={{ color: 'var(--qa-accent)' }}>Analysis</span>
          </h1>
          <div style={{ fontSize: '15px', color: 'var(--qa-text-mid)', fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginBottom: '64px' }}>QA management for software testing teams.</div>
            ...
            {([
              ...
              <div key={card.id} onClick={() => onChoose(card.id as any)}
                ...
                style={{
                  ...
                  boxShadow: hov === card.id ? '0 12px 40px -10px rgba(59,130,246,0.45)' : 'none',
                }}>
                <span style={{ position: 'absolute', top: '22px', right: '22px', fontSize: '16px', color: hov === card.id ? 'var(--qa-accent)' : 'var(--qa-text-faint)', transition: 'color .25s', fontFamily: "var(--font-mono, 'JetBrains Mono', monospace)" }}>↗</span>
                <div style={{ width: '56px', height: '56px', borderRadius: '12px', background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '28px' }}>
                  <card.Icon size={28} color={card.color} />
                </div>
                <div style={{ fontSize: '20px', fontWeight: 700, color: 'var(--qa-text)', marginBottom: '8px', fontFamily: "var(--font-heading, 'Montserrat', sans-serif)" }}>{card.title}</div>
                <div style={{ fontSize: '13px', color: 'var(--qa-text-mid)', fontFamily: "var(--font-body, 'Manrope', sans-serif)", lineHeight: 1.6 }}>{card.sub}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: '56px', display: 'flex', justifyContent: 'center' }}>
            <button
              ...
              style={{ background: 'none', border: 'none', color: 'var(--qa-accent)', fontFamily: "var(--font-body, 'Manrope', sans-serif)", fontSize: '13px', cursor: 'pointer', padding: '4px 2px', textDecoration: 'underline', textUnderlineOffset: '3px', transition: 'opacity .2s', letterSpacing: '.01em' }}
      ...
      <div style={{ position: 'fixed', bottom: '24px', right: '24px', zIndex: 100, display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div onClick={toggleTheme} style={{
          ...
          background: isDark ? '#3B82F6' : 'rgba(59,130,246,0.3)',
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative' }}>
      ...
      <div className="fu" style={{ width: '100%', maxWidth: '400px', position: 'relative', zIndex: 1 }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: C.yellow, cursor: 'pointer', fontFamily: "var(--font-body, 'Manrope', sans-serif)", fontSize: '12px', fontWeight: '700', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '6px', textShadow: `0 0 12px ${C.yellow}80` }}>← Back</button>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          <div style={{ fontSize: '22px', fontWeight: '800', fontFamily: "var(--font-heading, 'Montserrat', sans-serif)", background: `linear-gradient(135deg,${C.text},${C.accent})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Quality Analysis</div>
          <div style={{ fontSize: '11px', color: C.textDim, fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginTop: '5px', letterSpacing: '0.1em', textTransform: 'uppercase' }}>QA Team Sign In</div>
        </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', position: 'relative' }}>
      ...
      <div className="fu" style={{ width: '100%', maxWidth: '380px', position: 'relative', zIndex: 1, textAlign: 'center' }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: C.yellow, cursor: 'pointer', fontFamily: "var(--font-body, 'Manrope', sans-serif)", fontSize: '12px', fontWeight: '700', marginBottom: '28px', display: 'block', textShadow: `0 0 12px ${C.yellow}80` }}>← Back</button>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}><DevIcon size={52} color={C.purple} /></div>
        <h2 style={{ fontSize: '22px', fontWeight: '800', color: C.text, fontFamily: "var(--font-heading, 'Montserrat', sans-serif)", marginBottom: '8px' }}>What's your name?</h2>
        <p style={{ fontSize: '12px', color: C.textMid, fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginBottom: '28px', lineHeight: '1.7' }}>Your name appears on bug comments<br />and test case notes</p>
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '40px 20px', position: 'relative' }}>
      ...
      <div className="fu" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: C.yellow, cursor: 'pointer', fontFamily: "var(--font-body, 'Manrope', sans-serif)", fontSize: '12px', fontWeight: '700', marginBottom: '20px', display: 'block', margin: '0 auto 20px', textShadow: `0 0 12px ${C.yellow}80` }}>← Back</button>
        <div style={{ display: 'inline-block', padding: '4px 14px', borderRadius: '20px', border: `1px solid ${C.accent}40`, background: `${C.accent}0e`, fontSize: '10px', color: C.accent, fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginBottom: '14px' }}>Developer View · {devName}</div>
        <h2 style={{ fontSize: '24px', fontWeight: '800', color: C.text, fontFamily: "var(--font-heading, 'Montserrat', sans-serif)", marginBottom: '6px' }}>Choose your project QA</h2>
        <p style={{ fontSize: '11px', color: C.textMid, fontFamily: "var(--font-body, 'Manrope', sans-serif)", marginBottom: '36px' }}>View projects · Comment on test cases · Update bug status & add dev comments</p>
        <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', justifyContent: 'center' }}>
          {QA_LIST.map(q => (
            <div key={q.name} onClick={() => onChoose(q)}
              ...
              <div style={{ fontSize: '13px', fontWeight: '700', color: C.text, marginBottom: '4px', fontFamily: "var(--font-heading, 'Montserrat', sans-serif)" }}>{q.name}</div>
              <div style={{ fontSize: '10px', color: q.color, fontFamily: "var(--font-body, 'Manrope', sans-serif)" }}>{q.role}</div>
            </div>
          ))}
        </div>
      </div>
